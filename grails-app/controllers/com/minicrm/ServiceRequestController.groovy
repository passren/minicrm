...
class ServiceRequestController {
	...
	def listServiceRequests() {
		...
		if(searchCriteria.size() == 0) {
			serviceRequests = ServiceRequest.list(params)
		} else {
			serviceRequests = serviceRequestService.findServiceRequestByCriteria(searchCriteria)
		}
	...
	def viewServiceRequest() {
		def serviceRequest = ServiceRequest.get(params.id)
	...
	def saveServiceRequest() {
		...
		if (params.actionFlag == ConstUtils.CONTROLLER_ACTION_FLAG_ADD) {
			...
			serviceRequest = ServiceRequest.get(params.id)
